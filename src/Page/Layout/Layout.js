import React from "react";
import Navigation from "../../components/Navigation";
import styles from "./Layout.module.css";
import Spline from "@splinetool/react-spline";

const Layout = () => {
  const technologies = [
    { name: "HTML", color: "#E34F26", logo: "image/logoTech/html.png" },
    { name: "CSS", color: "#1572B6", logo: "image/logoTech/css.png" },
    { name: "Java", color: "#F7DF1E", logo: "image/logoTech/java.png" },
    { name: "React", color: "#61DAFB", logo: "image/logoTech/react.png" },
    {
      name: "TailwindCSS",
      color: "#38B2AC",
      logo: "image/logoTech/tailwind.png",
    },
    { name: "NodeJS", color: "#38B2AC", logo: "image/logoTech/node.png" },
  ];
  const repeatedTechnologies = [...technologies, ...technologies];
  const cards = [
    { index: 0, color: "142, 249, 252", image: "image/avatar/giang.jpg" },
    { index: 1, color: "142, 252, 204", image: "image/avatar/ngan.jpg" },
    { index: 2, color: "142, 252, 157", image: "image/avatar/thu.jpg" },
    { index: 3, color: "215, 252, 142", image: "image/avatar/ngoc.jpg" },
    { index: 4, color: "252, 252, 142", image: "image/avatar/duong1.jpg" },
    { index: 5, color: "252, 208, 142", image: "image/avatar/ngan.jpg" },
    { index: 6, color: "252, 142, 142", image: "image/avatar/thu.jpg" },
    { index: 7, color: "252, 142, 239", image: "image/avatar/ngoc.jpg" },
    { index: 8, color: "204, 142, 252", image: "image/avatar/khang.jpg" },
    { index: 9, color: "142, 202, 252", image: "image/avatar/duong2.jpg" },
  ];
  return (
    <>
      {/* Phần này giới thiệu chung */}
      <div className="bg-black h-screen flex text-white justify-center items-end">
        <div className={styles.tech}>
          <div className="Header flex justify-between">
            <div className=" w-[420px] mb-10 h-[87px] bg-[#000000] rounded-ee-2xl flex items-center justify-start">
              <div className="w-[420px] h-[80px] bg-[#ffffff59] mr-2 mb-2 rounded-ee-[10px] rounded-ss-[10px] flex justify-center ">
                <div className={`${styles.container}`}>
                  <div className={styles.scroll}>
                    {repeatedTechnologies.map((tech, index) => (
                      <div
                        key={index}
                        className={styles.techCircle}
                        aria-hidden={index >= technologies.length} // Ẩn các mục nhân đôi khỏi screen readers
                      >
                        <img
                          src={tech.logo}
                          alt={tech.name}
                          className={styles.techLogo}
                        />
                        {/* <div className="absolute top-[52px]">{tech.name}</div> */}
                      </div>
                    ))}
                  </div>
                </div>
              </div>
            </div>
            <div className=" w-[420px] mb-10 h-[87px] bg-[#000000] rounded-es-2xl flex items-center justify-start">
              <div className="w-[420px] h-[80px] bg-[#ffffff59] mr-2 mb-2 rounded-se-[10px] rounded-es-[10px] flex justify-center  ">
                <div className={styles.navi}>
                  <ul className="flex justify-between">
                    <li className={styles.iso}>
                      <span></span>
                      <span></span>
                      <span></span>
                      <a href="https://www.facebook.com/TruongGiangNguyen0901">
                        <svg
                          viewBox="0 0 320 512"
                          xmlns="http://www.w3.org/2000/svg"
                          className={styles.svg}>
                          <path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z"></path>
                        </svg>
                      </a>
                      <div className={styles.text}>Facebook</div>
                    </li>
                    <li className={styles.iso}>
                      <span></span>
                      <span></span>
                      <span></span>
                      <a href="">
                        <svg
                          viewBox="0 0 448 512"
                          xmlns="http://www.w3.org/2000/svg"
                          className={styles.svg}>
                          <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"></path>
                        </svg>
                      </a>
                      <div className={styles.text}>Zalo</div>
                    </li>
                    <li className={styles.iso}>
                      <span></span>
                      <span></span>
                      <span></span>
                      <a href="">
                        <svg
                          viewBox="0 0 16 16"
                          className={styles.svg}
                          xmlns="http://www.w3.org/2000/svg">
                          {" "}
                          <path d="M13.545 2.907a13.227 13.227 0 0 0-3.257-1.011.05.05 0 0 0-.052.025c-.141.25-.297.577-.406.833a12.19 12.19 0 0 0-3.658 0 8.258 8.258 0 0 0-.412-.833.051.051 0 0 0-.052-.025c-1.125.194-2.22.534-3.257 1.011a.041.041 0 0 0-.021.018C.356 6.024-.213 9.047.066 12.032c.001.014.01.028.021.037a13.276 13.276 0 0 0 3.995 2.02.05.05 0 0 0 .056-.019c.308-.42.582-.863.818-1.329a.05.05 0 0 0-.01-.059.051.051 0 0 0-.018-.011 8.875 8.875 0 0 1-1.248-.595.05.05 0 0 1-.02-.066.051.051 0 0 1 .015-.019c.084-.063.168-.129.248-.195a.05.05 0 0 1 .051-.007c2.619 1.196 5.454 1.196 8.041 0a.052.052 0 0 1 .053.007c.08.066.164.132.248.195a.051.051 0 0 1-.004.085 8.254 8.254 0 0 1-1.249.594.05.05 0 0 0-.03.03.052.052 0 0 0 .003.041c.24.465.515.909.817 1.329a.05.05 0 0 0 .056.019 13.235 13.235 0 0 0 4.001-2.02.049.049 0 0 0 .021-.037c.334-3.451-.559-6.449-2.366-9.106a.034.034 0 0 0-.02-.019Zm-8.198 7.307c-.789 0-1.438-.724-1.438-1.612 0-.889.637-1.613 1.438-1.613.807 0 1.45.73 1.438 1.613 0 .888-.637 1.612-1.438 1.612Zm5.316 0c-.788 0-1.438-.724-1.438-1.612 0-.889.637-1.613 1.438-1.613.807 0 1.451.73 1.438 1.613 0 .888-.631 1.612-1.438 1.612Z"></path>{" "}
                        </svg>
                      </a>
                      <div className={styles.text}>Discord</div>
                    </li>
                    <li className={styles.iso}>
                      <span></span>
                      <span></span>
                      <span></span>
                      <a href="https://www.instagram.com/g.jang91/">
                        <svg
                          className={styles.svg}
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 448 512">
                          <path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"></path>
                        </svg>
                      </a>
                      <div className={styles.text}>Instagram</div>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          {/* vòng xoay */}

          <div className={styles.wrapper}>
            <div
              className={styles.inner}
              style={{ "--quantity": cards.length }}>
              {cards.map((card) => (
                <div
                  key={card.index}
                  className={styles.card}
                  style={{
                    "--index": card.index,
                    "--color-card": card.color,
                    backgroundImage: `url(${card.image})`, // Thêm ảnh nền từ URL
                  }}>
                  <div className={styles.img}></div>
                </div>
              ))}
            </div>
          </div>
        </div>
        <div className=" w-[420px] mb-10 h-[87px] bg-[#222327] rounded-3xl flex items-center justify-center fixed z-50">
          <div className={styles.borderGradient}>
            <Navigation />
          </div>
        </div>
      </div>
      {/* Phần này cho tech */}
      <div className="bg-[#000000] h-screen flex text-white justify-center items-end">
        <div className={styles.tech}>
          <main>
            <Spline scene="https://prod.spline.design/nz5Pe37qfm45Ipts/scene.splinecode" />
          </main>
        </div>
      </div>
    </>
  );
};

export default Layout;
